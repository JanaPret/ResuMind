    ...
    <main className="grid gap-8">
      ...
      >
        <label className="grid gap-2">
          ...
          <textarea className="h-40 resize-y rounded-xl border border-gray-200 bg-white/70 p-3 outline-none focus:ring-2" style={{ boxShadow: '0 0 0 2px hsl(var(--accent) / 0.25)' }} placeholder="Paste your résumé text" {...register('masterText', { required: true })} />
        </label>
        <label className="grid gap-2">
          <span className="text-sm font-medium">Job description</span>
          <textarea className="h-40 resize-y rounded-xl border border-gray-200 bg-white/70 p-3 outline-none focus:ring-2" style={{ boxShadow: '0 0 0 2px hsl(var(--secondary) / 0.25)' }} placeholder="Paste the job ad" {...register('jobText', { required: true })} />
        </label>
        <div className="grid gap-2 sm:flex sm:items-center sm:gap-4">
          <label className="grid gap-1">
            <span className="text-sm font-medium">Tone</span>
            <select className="rounded-xl border border-gray-200 bg-white/70 p-2 outline-none focus:ring-2" style={{ boxShadow: '0 0 0 2px hsl(var(--primary) / 0.25)' }} {...register('tone')}>
            ...
            <div className="grid gap-1">
              ...
              <div className="flex flex-wrap gap-2">
                {result.keywords.map((k) => (
                  <span key={k} className="rounded-full px-2 py-1 text-xs text-gray-700 ring-1 ring-inset" style={{
                    background: 'linear-gradient(135deg, hsl(var(--secondary) / 0.08), hsl(var(--accent) / 0.08))',
                    borderColor: 'hsl(var(--secondary) / 0.15)'
                  }}>
                    {k}
                  </span>
                ))}
      ...
      <section className="card grid gap-4">
        ...
        <div className="grid gap-3 md:grid-cols-2">
          <label className="grid gap-2">
            ...
            <textarea className="h-40 resize-y rounded-xl border border-gray-200 bg-white/70 p-3 outline-none focus:ring-2" style={{ boxShadow: '0 0 0 2px hsl(var(--accent) / 0.25)' }} placeholder="Paste your current resume text" value={atsInput} onChange={(e) => setAtsInput(e.target.value)} />
          </label>
          <label className="grid gap-2">
            <span className="text-sm font-medium">Optional job description</span>
            <textarea className="h-40 resize-y rounded-xl border border-gray-200 bg-white/70 p-3 outline-none focus:ring-2" style={{ boxShadow: '0 0 0 2px hsl(var(--secondary) / 0.25)' }} placeholder="Paste the target job description (optional)" value={jobInput} onChange={(e) => setJobInput(e.target.value)} />
          </label>
        </div>
        <div>
        ...
        {ats && (
          <div className="grid gap-3">
            <div className="flex items-center gap-3">
              <div className="h-2 w-32 rounded" style={{ backgroundColor: 'hsl(var(--muted))' }}>
                <div className="h-2 rounded" style={{ width: `${ats.atsScore}%`, background: 'linear-gradient(90deg, hsl(var(--primary)), hsl(var(--accent)))' }} />
              </div>
              ...
              <div>
                ...
                <div className="mt-1 flex flex-wrap gap-2">
                  {ats.issues.missingSections.map((s: string) => (
                    <span key={s} className="rounded-full px-2 py-1 text-xs ring-1" style={{
              ...
              <div>
                ...
                <div className="mt-1 flex flex-wrap gap-2">
                  {ats.issues.keywordGaps.slice(0, 12).map((k: string) => (
                    <span key={k} className="rounded-full px-2 py-1 text-xs ring-1" style={{
